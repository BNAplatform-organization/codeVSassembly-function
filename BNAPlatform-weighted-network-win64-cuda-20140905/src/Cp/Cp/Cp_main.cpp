...
int main(int argc, char * argv[])
{
	...
	if (argc < 3 || argc > 4) 
	{
		cerr<<"Input format: .\\Cp.exe dir_for_csr num_of_random_networks (option)Cp_algorithm_type \nFor example: .\\Cp.exe d:\\data 10 1(option)"<<endl;
		exit(1);	
	}

	int Cptype = 2;
	if (argc == 3 || argv[3][0] == '2') ;
	...
	for (int i = 0; i < FileNumber; i++)
	{
		...
		for (l = 0; l < Maslov_num; l++)
		{
			...
			if (Cptype == 2)
				Maslov_mean_Cp[l] = Cp_2(C_dst, R_dst, V_dst, Cp_result, N);
			else
